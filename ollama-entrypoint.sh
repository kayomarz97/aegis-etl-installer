...
# 2. Wait for ollama serve to bind.
# Ollama binds in <1s (confirmed from logs). We sleep 5s to be safe, then
# verify the process is still alive. Avoids curl IPv4/IPv6 loopback issues
# entirely — the serve process either started or it didn't.
echo "[ollama-entrypoint] Waiting for ollama serve to bind..."
sleep 5
if ! kill -0 "$OLLAMA_PID" 2>/dev/null; then
    echo "[ollama-entrypoint] ERROR: ollama serve exited unexpectedly"
    exit 1
fi
